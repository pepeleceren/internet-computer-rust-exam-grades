...
thread_local! {
    ...
    static EXMAP_MAP: RefCell<StableBTreeMap<u64, Exam, Memory>> = RefCell::new(StableBTreeMap::init(
...
fn get_exam(key: u64) -> Option<Exam> {
    EXMAP_MAP.with(|p| p.borrow().get(&key))
}
...
fn insert_exam(key: u64, value: Exam) -> Option<Exam> {
    EXMAP_MAP.with(|p| p.borrow_mut().insert(key, value))
}
